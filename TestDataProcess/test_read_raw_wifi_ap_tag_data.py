...
def arima_model(wifi_tag_passenger):
    ...
    print order
    # results_ARIMA = sm.tsa.ARIMA(wifi_tag_passenger, order=(7,0,0)).fit(
    ...
    fig = results_ARIMA.predict().plot(ax=ax)
    # fig = results_ARIMA(start, end, ax=ax, dynamic=True, plot_insample=False)
    plt.xlabel("time")
    plt.ylabel("passagerCounts")
    plt.show()


def read_wifi_tag_data(wifi_tag_file):
...
#
read_wifi_tag_data("../RawData/WIFI_AP_RAW_Data/E1-1A-2<E1-1-02>.csv")
